...
const input = { service: 'payments', level: 'error' as const, message: 'Payment timeout', timestamp: '2026-08-26T10:00:00Z' };
...
describe('POST /logs', () => {
  ...
  it.each([
    { level: 'debug' }, { message: '' }, { message: 'x'.repeat(8001) },
    { timestamp: 'yesterday' }, { timestamp: '2026-08-26' }, { service: '../invalid' }, { extra: true },
  ])('rejects invalid input: %j', async (change) => {
    expect((await request(app).post('/logs').send({ ...input, ...change })).status).toBe(400);
  ...
  it('rejects non-JSON, malformed JSON and oversized bodies', async () => {
    ...
    expect((await request(app).post('/logs').send({ ...input, message: 'x'.repeat(20000) })).status).toBe(413);
  });
...
describe('read API', () => {
  it('combines filters and caps the requested window', async () => {
    const response = await request(app).get('/logs').query({ service: 'payments', level: 'error', q: 'timeout', limit: 25 });
    expect(response.status).toBe(200);
    expect(repository.list).toHaveBeenCalledWith({ service: 'payments', level: 'error', q: 'timeout', limit: 25 });
    expect(response.body.logs).toEqual([record]);
  });
  it.each(['limit=0', 'limit=201', 'limit=no', 'level=debug', 'level=info&level=error', 'unknown=1'])('rejects invalid query %s', async (query) => {
